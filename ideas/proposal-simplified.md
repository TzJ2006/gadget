...
审计还查出**想法图本身有 32 处已经和代码漂移**（第七节），其中两处值得现在就知道：
...
| I-045 的硬阻塞 | 「拆分前必须先从 git 历史里清理曾提交过的私人内容与 secrets」，所以受阻 | 这条被**量错了**，而量错正是它一直卡在受阻的原因：实测历史只有 **20 个提交**（4b800f4 → 43a29db），历史上出现过 818 个路径，真正要摘掉的只有 `docs/ecl/dag/` 下 **3 个文件**。一次 gitleaks 当闸门 + 一次对 3 个路径的 `filter-repo`——几小时，不是一个项目 |
