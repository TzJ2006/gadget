...
5. ~~**`daily_summary.py`（110 行）与 `common/html_text.py`（101 行）还在用吗？**~~
   **复核后这条已经可以回答，不必你判断：**
   `common/html_text.py` **是活的**——`tools/research/homepage_discovery.py:13` 与
   `tools/research/apis/arxiv_client.py:12` 都 import 了 `HTMLTextExtractor`，
   `arxiv_client.py:248` 用它把下载来的 arXiv HTML 转成文本。留。
   `tools/summarize/daily_summary.py` **没有任何生产调用方**——它是个纯 re-export shim，
   注释里写明消费者是 `mcp_server.py`，而仓里没有这个文件；
   `tools/summarize/tests/test_imports.py:69-71` 说周报月报依赖它也是错的（两份都没提它）。
   这和提案已经点出的 `research_scout.py` shim 是同一种「为不存在的消费者保命」。
   **所以真正要你拍板的只剩一件：删不删 `daily_summary.py` 和它那个测试。**
...
**还没做的**：第十一节那六件要你拍板的事一件都没动（`anthropic` 后端去留、
