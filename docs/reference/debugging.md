...
- **LLM backend** — `DEFAULT_BACKEND` is `os.environ.get("GADGET_LLM_BACKEND") or "ollama"` (`llm.py:53`; empty env still falls back to `ollama`). Per-call `backend` / `--api` overrides. An unknown backend raises `ValueError` (valid: `LLM_BACKENDS` at `llm.py:58`) instead of silently routing elsewhere (`claude_cli`, the backend that fallthrough used to reach, has itself been removed).
