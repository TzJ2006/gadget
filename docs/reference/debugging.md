...
- **summarize paths** — `_resolve_output_dir(cli, env_key, config_key, default)` (`config.py:53-68`): CLI arg > env (`SUMMARIZE_LOGS_DIR` / `SUMMARIZE_REPORTS_DIR`) > config (`logs_dir`/`reports_dir`) > default under `outputs/`. Config file = `SUMMARIZE_CONFIG` env path if set (beats both lookups — test isolation / multi-config switching, `config.py:19-23`), else repo-local `tools/summarize/config.json` if present (`config.py:15`), else `~/.config/summarize/config.json` (`config.py:16`); resolved once at import into `_CONFIG_PATH` (`config.py:26`).
- **summarize behavior defaults** (`api`/`deploy`/`hugo_site`/`workers`) — `cli_defaults()` maps config keys to argparse dests via `parser.set_defaults` (`config.py:84-96`); an explicit flag still overrides.
- **summarize LLM/translation knobs** — `apply_env_from_config()` bridges config → env via `os.environ.setdefault` (`config.py:100-118`): `model→OLLAMA_MODEL`, `base_url→OLLAMA_BASE_URL`, `reasoning_effort→OPENAI_REASONING_EFFORT`, `translation_model→GADGET_TRANSLATION_MODEL`, `translation_model_ollama→OLLAMA_TRANSLATION_MODEL`, `translation_backend→GADGET_TRANSLATION_BACKEND`. Because it's `setdefault`, a real exported env var wins over config.
- **research** — `resolve_param(args, project, name, default)` (`scout/config.py:86-94`): CLI > `project.json[name]` > merged config `default_<name>` > default. Config merge (`config.py:59-83`) reads `~/.config/research_scout/config.json` then `~/.config/research/config.json`; scout keys win.
- **LLM backend** — `GADGET_LLM_BACKEND` (default `ollama`, empty falls back to `ollama`, `llm.py:50`); per-call `backend` param overrides. An unknown backend now raises `ValueError` (valid: `LLM_BACKENDS`) instead of silently running `claude_cli`.
- **Translation backend** — `GADGET_TRANSLATION_BACKEND` explicit > auto-detect ollama→vllm→llamacpp→transformers (`engine.py:734-778`; an unknown non-empty value raises `ValueError` instead of silently auto-selecting); model = arg > `GADGET_TRANSLATION_MODEL` > default (`engine.py:88-93`). The in-process engine cache is keyed by `(backend, model_id)`, so switching backend mid-process yields a fresh engine, not a stale one.

## Where logs live

| Tool | Path | Notes |
|---|---|---|
| summarize export | `outputs/logs/summarize/<date>_<device>.json` | export logs + usage snapshots (`daily.py:94`) |
| summarize merge | `outputs/logs/summarize/merge_logs/merge_<date>.log` | per-date merge subprocess stdout+stderr, opened `w` (`daily.py:299-315`) |
| summarize deploy | **`outputs/reports/logs/deploy.log`** | ⚠️ under `outputs/reports`, not `outputs/logs`; opened `w` — **truncated every deploy** (`daily.py:889-892`) |
...
| Cache | Path | Notes |
|---|---|---|
| summarize final report | `outputs/cache/summarize/<date>.json` | skipped by `--no-cache`/`--force` (`daily.py:631-634`) |
| summarize chunks | `outputs/cache/summarize/chunks/<date>/<hash>.json` | **auto-deleted** after final report cached (`daily.py:668` → `llm.py:436-439`); a crash leaves them behind |
...
1. **`create_engine()` evicts the resident Ollama chat model.** When the chosen translation engine is **not** `OllamaEngine`, `common/engine.py:782-783` calls `_free_ollama_vram()`, which POSTs `keep_alive:0` for every resident model — unloading e.g. `qwen3.6:35b`. The next summarize chat call then cold-reloads (~8 s). Fires on any deploy/translate step using llamacpp/transformers/vllm. To avoid: `GADGET_TRANSLATION_BACKEND=ollama`, or `GADGET_KEEP_OLLAMA=1` to keep the chat model resident regardless of backend.
2. **`apply_env_from_config()` mutates `os.environ` at startup** (`config.py:100-118`) for the whole run — affects `common.llm`/`common.engine` even in subcommands you didn't expect.
3. **`deploy.log` is truncated every run** (opened `w`, `daily.py:892`) and lives under `outputs/reports/logs/` — easy to miss.
4. **`_finalized` flags**: export marks past-date logs finalized (`daily.py:181-192`), merge marks past-date reports finalized (`:701-708`). Once set, re-running export/merge **silently skips** that date unless `--force` (`:234,368,602-604`).
