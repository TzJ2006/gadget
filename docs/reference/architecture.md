...
- **Chat backends** — `ollama` (default), `anthropic`, `openai` (single source of truth: `LLM_BACKENDS`, `llm.py:55`). Dispatched in `common/llm.py` by `call_llm_raw` (`:167`) and `call_llm` (`:350`); unknown values raise `ValueError`.
