...
- **Default chat backend** — `ollama`. `common/llm.py:50` `DEFAULT_BACKEND = os.environ.get("GADGET_LLM_BACKEND") or "ollama"` (the `or` form, so an exported-but-empty env var still falls back to ollama). Default served chat tag `qwen3.6:35b` (`llm.py:59`). Override per-call, via `--api`, or globally via `GADGET_LLM_BACKEND`.
...
- **Default translation model** — `tencent/Hy-MT2-1.8B` (`common/engine.py:19`). GGUF variant `tencent/Hy-MT2-1.8B-GGUF` (`:20`); Ollama tag `hf.co/tencent/Hy-MT2-1.8B-GGUF` (`:24`). (Docs said `HY-MT1.5-1.8B` — fixed in Phase 1.)
